...
export default function Navbar({ active }: NavbarProps) {
  ...
  const regularList = 'flex gap-x-2 py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-red-700 md:p-0';
        ...
        >
          <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white">
            <li>
              <Link
              href="/"
              className={active === 'home' ? activeList : `${regularList} group`}
              aria-current="page"
              >
                <svg 
                  xmlns="http://www.w3.org/2000/svg" 
                  className={`w-5 h-5 ${active === 'home' ? 'text-red-600' : 'text-gray-900 group-hover:text-red-700'}`}
                  viewBox="0 0 24 24" 
                  fill="none" 
                  stroke="currentColor" 
                  strokeWidth="2" 
            ...
            <li>
              <Link
              href="/form"
              className={active === 'form' ? activeList : `${regularList} group`}
              aria-current="page"
              >
              <svg 
                xmlns="http://www.w3.org/2000/svg" 
                className={`w-5 h-5 ${active === 'form' ? 'text-red-600' : 'text-gray-900 group-hover:text-red-700'}`}
                viewBox="0 0 24 24" 
                fill="none" 
                stroke="currentColor" 
                strokeWidth="2" 
